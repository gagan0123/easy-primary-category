...
( function ( $ ) {
	...
	function hasPrimaryTermElements( checkbox ) {
		return 1 === $( checkbox ).closest( "li" ).children( ".easy-make-primary-term" ).length;
	}
	...
	function getPrimaryTerm( taxonomyName ) {
		...
		primaryTermInput = $( "#easy-primary-" + taxonomyName );
	...
	function updatePrimaryTermSelectors( taxonomyName ) {
		...
		taxonomyListItem.removeClass( "easy-term-unchecked easy-primary-term easy-non-primary-term" );

		$( ".easy-primary-category-label" ).remove();
		taxonomyListItem.addClass( "easy-term-unchecked" );
		...
		checkedTerms.each( function ( i, term ) {
			...
			listItem.removeClass( "easy-term-unchecked" );
			...
			if ( term.val() === getPrimaryTerm( taxonomyName ) ) {
				listItem.addClass( "easy-primary-term" );

				label = term.closest( "label" );
				label.find( ".easy-primary-category-label" ).remove();
				label.append( primaryTermScreenReaderTemplate( {
					taxonomy: taxonomies[ taxonomyName ],
				} ) );
			} else {
				listItem.addClass( "easy-non-primary-term" );
			}
		} );
	}
	...
	$.fn.initYstSEOPrimaryCategory = function () {
		return this.each( function ( i, taxonomy ) {
			...
			metaboxTaxonomy.on( "click", ".easy-make-primary-term", makePrimaryHandler( taxonomy.name ) );
		} );
	};
